...
export const ProjectSelectOrCreate = () => {
  ...
  const set_page = useAppStore((state) => state.set_page);

  const set_selected_folder = useAppStore((state) => state.set_selected_folder);
  const set_selected_folder_contents = useAppStore(
    (state) => state.set_selected_folder_contents
  );

  const set_project_folder = useAppStore((state) => state.set_project_folder);
  const set_project_folder_contents = useAppStore(
    (state) => state.set_project_folder_contents
  );

  const set_template_file = useAppStore((state) => state.set_template_file);
  ...
  const handleClick_create_project_folder = async () => {
    ...
    set_selected_folder_contents(folder_contents);
  ...
  const handleClick_open_existing_folder = async () => {
    ...
    if (template_file) {
      set_project_folder(directoryHandle);
      const project_folder_contents = await get_directory_contents({
        directoryHandle,
      });
      ...
      set_selected_folder_contents(folder_contents);
      // no template, or project folder yet
      open__existing_project_modal(); // like a confirmation to redirect or cancel
      return;
    }
